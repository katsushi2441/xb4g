...
function g_page_giin(int $id, int $page): void
{
    ...
    echo g_insight_html(g_insight('giin', $g['slug']), '本人',
        '国会での発言と、提出者に名前がある議案' . ($L ? '、本人の公式Xと公式YouTubeの新着' : ''));
...
function g_page_theme(string $slug, int $page): void
{
    ...
    if ($nw) {
        ...
        echo '<div class="panel note">いま集めている範囲（国会に出された議案と、'
           . '省庁の報道発表）には、このことがらに当たる新しい動きがありません。'
           . '国会での発言は下に出ています。'
           . '<br><a href="' . g_url('news') . '">ほかのことがらの動きを見る</a></div>';
    }
...
function g_page_about(): void
{
    ...
    echo '<h1>このサイトについて</h1><div class="panel">'
       ...
       . '<h2 id="insight">「いまの時点でのAI考察」について</h2>'
       . '<p>議員のページと会派のページに出している考察は、'
       . '<b>そのページに出ている材料だけ</b>——国会での質疑、提出者に名前がある議案、'
       . '本人の公式Xと公式YouTubeの新着——をローカルのAIに渡して書かせた下書きです。'
       . '本人や会派の見解ではありません。</p>'
       . '<p><b>数字はAIに書かせていません。</b>件数・日付・割合は、'
       . 'すべて会議録の集計から画面が出しています。'
       . 'AIの文に「何件」「何％」のような数の主張が混ざっていたら、'
       . 'その文は<b>採用せずに捨てています</b>。数え間違いを文章の形で配らないためです。</p>'
       . '<p>あわせて、次のものが入った文も捨てています。</p>'
       . '<ul>'
       . '<li><b>人物の評価</b>——「熱心」「優れた」「積極的」など。'
       . '党派を問わず同じ道具で同じように作るので、褒め貶しが入ると中立が壊れます。</li>'
       . '<li><b>賛成・反対の判定</b>——会議録から機械判定できないので、書けば推測になります。</li>'
       . '<li><b>本人の発信の取り違え</b>——省庁の報道発表を「本人の公式発信」として'
       . '書いてしまう間違いです。実際に一度起きたので、段落の役割を固定して防いでいます。</li>'
       . '</ul>'
       . '<p class="note">3回書き直させても通らなかった場合は、'
       . '<b>そのページには考察を出しません。</b>無理に出すより、空のほうが誠実だと考えています。'
       . '使っているのは手元の機械で動く公開モデル（' . g_e(g_insight_model()) . '）で、'
       . '発言の本文を外部のサービスへ送ってはいません。</p>'
...
function g_page_party(string $slug): void
{
    ...
    echo g_insight_html(g_insight('party', $name), 'この会派',
        'この会派の議員の、国会での発言と提出議案');
